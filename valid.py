...
test_data = TissueData(root_dir, 'valid', transform = transform, metadata=False)
...
data.dump(open('experiments/{0}/pred_label_max_{0}_{1}_valid.npy'.format(opt.experiment,opt.model), 'wb'))

#This can be used if need to print the auc and save the roc curve automatically

roc_auc  = get_auc('experiments/{0}/images/valid_AUC_max_{1}.jpg'.format(opt.experiment,opt.model),
...
data = np.column_stack((np.asarray(predictions),np.asarray(labels)))
data.dump(open('experiments/{0}/pred_label_avg_{0}_{1}_valid.npy'.format(opt.experiment,opt.model), 'wb'))

#This can be used if need to print the auc and save the roc curve automatically
roc_auc  = get_auc('experiments/{0}/images/valid_AUC_avg_{1}.jpg'.format(opt.experiment,opt.model),
                   predictions, labels, classes = range(num_classes))
